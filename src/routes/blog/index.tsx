          ...
          <div key={year} className="mt-16">
            ...
            <ul className="mt-10 space-y-12">
              {yearPosts.map((post, idx) => {
                ...
                const nextPost = yearPosts[idx + 1]
                const blipCount = nextPost ? countBlipsBetween(nextPost.date, post.date) : 0
                // Special posts get custom card layouts per slug
                if (post.slug === 'chat-control-eu' && img) {
                  return (
                    <div key={post.slug}>
                      <SurveillanceCard post={post} img={img} dayOf={dayOf} monthOf={monthOf} />
                ...
                return (
                  <div key={post.slug}>
                    <li data-post className="group grid gap-x-6 gap-y-4 md:grid-cols-12 md:items-start">
