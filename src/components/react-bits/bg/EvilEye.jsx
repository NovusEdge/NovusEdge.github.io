...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    function onMouseMove(e) {
      ...
      // clamp to the socket: when the eye is scaled small its rect is tiny, so an
      // off-eye cursor would otherwise push the pupil far past its range. The
      // bound is tight because uMouse is multiplied by pupilFollow in the shader.
      const B = 0.42;
    ...
    // listen on window, not the container: the eye wrapper is pointer-events-none
    // so it never blocks the page, and the pupil tracks the cursor everywhere.
