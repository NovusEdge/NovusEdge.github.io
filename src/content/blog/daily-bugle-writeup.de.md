...
description: "Fingerprinting und Exploiten einer Joomla-Installation mit Metasploit, um Fuß zu fassen und im TryHackMe-Room Daily Bugle Privilegien zu eskalieren."
---
...
Mal schauen, ob wir mit Metasploit etwas finden können:
```shell-session
$ sudo msfconsole -q
msf6 > search joomla
...
...
Eine kurze Suche auf ExploitDB nach Joomla 3.7 liefert einen netten Exploit, den wir nutzen können: https://www.exploit-db.com/exploits/42033
...
Machen wir bei der Gelegenheit auch gleich noch etwas Directory Enumeration:
```shell-session
...
```
Wenn wir den Pfad `/administrator` aufrufen, landen wir auf folgender Seite:
![](/assets/img/writeup_assets/daily-bugle/login-page.png)

Laut der [Joomla Documentation](https://docs.joomla.org/Administrator_(User)):
> The default Super Users is admin and is installed with every Joomla! site.

Eine weitere kurze Suche mit searchsploit liefert uns einen praktischen Exploit für SQLi in Joomla 3.7.0:
```shell-session
...
Wenn wir diesen Exploit durchgehen, sehen wir, dass er uns einen praktischen `sqlmap`-Befehl liefert:
```shell-session
$ sqlmap -u "http://localhost/index.php?option=com_fields&view=fields&layout=modal&list[fullordering]=updatexml" --risk=3 --level=5 --random-agent --dbs -p list[fullordering]
```

Indem wir die Flags `--tables` und `-D joomla` hinzufügen, können wir angeben, dass wir zuerst die Tabellen der Datenbank verstehen wollen und welche Art von Datenbank wir angreifen.
```shell-session
...
Das liefert uns insgesamt 72 Tabellen – die meisten davon sind nicht besonders nützlich, aber eine, die wir gebrauchen können, ist die Tabelle `__users`:
```shell-session
...
Nice! Versuchen wir jetzt, die Spalten `username` und Password zu dumpen:
```shell-session
...
Okay, wir haben also einen Benutzernamen und einen Passwort-Hash, knacken wir ihn. (Sieht nach einem Blowfish-Hash aus):
```shell-session
...
Damit erhalten wir Zugriff auf das Control Panel des Joomla-CMS auf unserem Target. Als Nächstes nutzen wir das für den initialen Foothold...

## Gaining Access

Navigiere im User-Dashboard zum Templates-Menü und versuche, eine PHP Reverse Shell hochzuladen. Bei mir hat das nicht geklappt, also habe ich alternativ einfach den Inhalt von `index.php` im Standard-Template editiert, sprich: den Reverse-Shell-Code dort hineinkopiert. 


Wenn wir einen Listener auf unserer Maschine starten und die hochgeladene Payload aufrufen (in diesem Fall `index.html`), bekommen wir eine feine Reverse Shell:
...
Stabilisieren wir die Shell, bevor wir weitermachen:
```shell-session
...
bash-4.2$ 
```
Nice! Jetzt können wir mit dem Üblichen weitermachen:
```shell-session
bash-4.2$ whoami
apache
bash-4.2$ uname -a
...
Nutzen wir `linpeas`, um uns das Leben leichter zu machen:
```shell-session
...
Tja, wir haben ein Passwort. Kombinieren wir das mit dem Benutzernamen `jjameson` (bruh, mach einfach `ls` im Verzeichnis `/home` -\_-) und versuchen wir, uns per `ssh` auf der Maschine einzuloggen:
```shell-session
...
Nice! Kommen wir nun zur Privilege Escalation...

## Privilege Escalation

Versuchen wir zuerst etwas Enumeration:
```shell-session
...
OOOOOOOO, WIR HABEN EINEN TREFFER, LEUTE!!! Ein Blick ins gute alte GTFObins und die Suche nach `yum` zeigt uns 2 Optionen, um über `sudo` eine Root-Shell zu bekommen:
...
## Conclusion
Wenn dir dieses Writeup hilft, folge mir gerne auf [github](https://github.com/NovusEdge) und/oder lass einen Star auf dem Repository da: https://github.com/NovusEdge/thm-writeups
