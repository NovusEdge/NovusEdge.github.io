...
description: "TryHackMeのDaily Bugleルームで、Metasploitを使ってJoomlaのフィンガープリント採取とエクスプロイトを行い、足がかりを得て権限昇格する手順。"
---

## セットアップ 

まずはTryHackMeのVPNサーバーに接続する必要がある。詳細については[Access](https://tryhackme.com/access)ページを確認してほしい。
...
## 偵察（Reconnaissance）
サクッとポートスキャンと情報収集をやっていこう（`rustscan`の作者に圧倒的感謝）：
```shell-session
...
ExploitDBでJoomla 3.7をさっと検索してみると、使えそうなナイスなエクスプロイトが見つかった：https://www.exploit-db.com/exploits/42033
...
```
`/administrator`のパスにアクセスすると、以下のページが表示される：
![](/assets/img/writeup_assets/daily-bugle/login-page.png)

[Joomla Documentation](https://docs.joomla.org/Administrator_(User))によると：
> デフォルトのスーパーユーザーは admin で、すべての Joomla! サイトに標準でインストールされます。

さらにsearchsploitでサクッと検索すると、Joomla 3.7.0のSQLiに使える良さげなエクスプロイトが出てくる：
```shell-session
...
このエクスプロイトを読んでみると、親切にも`sqlmap`のコマンドが載っている：
```shell-session
$ sqlmap -u "http://localhost/index.php?option=com_fields&view=fields&layout=modal&list[fullordering]=updatexml" --risk=3 --level=5 --random-agent --dbs -p list[fullordering]
```

`--tables`と`-D joomla`フラグを追加することで、まずはデータベースのテーブル構造を把握したいこと、そしてどのデータベースを攻撃対象にするかを指定できる。
```shell-session
...
これで合計72個のテーブルが取得できた。大半は使い物にならないが、使えるテーブルの1つが`__users`テーブルだ：
```shell-session
...
いいね！じゃあ`username`とパスワードのカラムをダンプしてみよう：
```shell-session
...
よし、ユーザー名とパスワードハッシュが手に入ったのでクラックしてみよう。（Blowfishのハッシュっぽい）：
```shell-session
...
これを使ってターゲット上のJoomla CMSコントロールパネルにアクセスできる。次はこれを利用して最初の足がかり（initial foothold）を掴んでいく...

## アクセス奪取（Gaining Access）

ユーザーダッシュボードからテンプレートメニューに移動し、PHPリバースシェルをアップロードしてみる。自分の環境ではうまくいかなかったので、代替策としてデフォルトテンプレートの`index.php`の中身を編集して、そこにリバースシェルのコードを直接コピペした。


ローカルマシンでリスナーを起動し、アップロードしたペイロード（今回は`index.html`）にアクセスすると、無事にリバースシェルが返ってきた：
...
先に進む前にシェルを安定化（stabilize）させておこう：
```shell-session
...
さて、パスワードが手に入ったので、ユーザー名`jjameson`（おいおい、`/home`ディレクトリを`ls`しただけだぞ -\_-）と組み合わせて、`ssh`でマシンへのログインを試してみよう：
```shell-session
...
おおおお、当たり引いたぞみんな！！！頼れる相棒GTFObinsに行って`yum`を検索すると、`sudo`を使ってrootシェルを取るための2つの選択肢が見つかる：
...
## まとめ
このWriteupが役に立ったら、ぜひ[GitHub](https://github.com/NovusEdge)のフォローやリポジトリへのスターをお願いします：https://github.com/NovusEdge/thm-writeups


- ルーム：[Daily Bugle](https://tryhackme.com/room/dailybugle)
